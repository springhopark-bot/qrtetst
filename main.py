...
<script>
    ...
    function stopCharging(isCompleted = false) {{
        ...
        let initialAmount = selectedAmount;
        let usedCost = Math.round(currentKwh * UNIT_PRICE);
        let cancelAmount = initialAmount - usedCost;
...
async def reset_payment(req: Optional[ResetRequest] = None):
    ...
    if req and req.shop_order_no:
        order_db.pop(req.shop_order_no, None)
    return {"resCd": "0000", "resMsg": "초기화 완료"}
