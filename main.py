    ...
    try:
        ...
        if res_cd == "0000":
            if shop_order_no in order_db:
                ...
                # 메모리에 주문번호가 없더라도 KICC에는 수신 성공했다고 응답해야 재전송을 멈춤
                print(f"⚠️ [경고] order_db에 주문번호 없음: {shop_order_no}")

        # KICC 서버에 "정상적으로 수신함"을 알리는 성공 응답 (HTTP 200)
        return {"resCd": "0000", "resMsg": "SUCCESS"}

    except Exception as e:
        print(f"❌ [웹훅 처리 중 에러 발생]: {e}")
        # 예외가 발생하더라도 KICC 재전송 폭주를 막기 위해 일단 200 OK 응답 반환
        return {"resCd": "0000", "resMsg": "SUCCESS"}
